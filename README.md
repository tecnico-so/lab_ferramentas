...
Para navegar nas páginas do manual podem ser usadas as setas do teclado e as teclas _PageUp_ e
_PageDown_.
...
O `make` é, em particular, útil para compilar _software_ pois é capaz de, a partir das dependências descritas na `Makefile`, determinar quais os ficheiros (alvo) que estão desatualizados e precisam de ser recompilados.
...
Notar o uso de variáveis, com atribuição de valor: ``CFLAGS = -g -Wall -std=c17`` e acesso ao seu conteúdo: ``$(CFLAGS)``
...
5. Notar que algumas regras não possuem dependências.
